...
export async function saveHotelioDemo(data) {
  ...
  const result = await query(
    `INSERT INTO hotelio_demo_requests (full_name, hotel_name, city, room_count, phone, email, position, modules, note, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'new')`,
    [data.fullName, data.hotelName, data.city, data.roomCount ? Number(data.roomCount) : null, data.phone, data.email, data.position || null, (data.modules || data.interests)?.length ? JSON.stringify(data.modules || data.interests) : null, data.note || null]
  );
