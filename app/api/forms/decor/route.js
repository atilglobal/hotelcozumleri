...
import { validateDecorForm } from "@/lib/validation";
...
export async function POST(request) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const files = formData.getAll("files").slice(0, MAX_FILES);
      if (files.length) {
        const uploadDir = path.join(process.cwd(), "uploads", "decor");
    ...
    const errors = validateDecorForm(data);
    ...
    return NextResponse.json({
      success: true,
      message: "Dekorasyon teklif talebiniz alındı. En kısa sürede sizinle iletişime geçeceğiz.",
    });
  } catch {
    return NextResponse.json(
