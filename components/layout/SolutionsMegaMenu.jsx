  ...
  return (
    ...
    >
      ...
      {open && (
        <>
          <div
            className="fixed inset-x-0 bottom-0 z-40 bg-[#040810]/80 backdrop-blur-[2px]"
            style={{ top: "var(--header-height)" }}
            aria-hidden="true"
          />

          <div
            className="absolute top-full left-1/2 -translate-x-1/2 pt-4 w-[720px] max-w-[90vw] z-[60]"
            role="menu"
            onMouseEnter={openMenu}
            onMouseLeave={closeMenu}
          >
            <div className="rounded-xl shadow-[0_24px_80px_rgba(0,0,0,0.65)] overflow-hidden border border-white/12 bg-[#0B1424]">
              <div className="grid grid-cols-5">
                <div className="col-span-2 bg-[#07101C] p-6 md:p-8 border-r border-white/8">
                ...
                <div className="col-span-3 p-4 md:p-6 bg-[#0B1424]">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                    {megaMenuSolutions.items.map((item) => (
                      <Link
                        ...
                        href={item.href}
                        role="menuitem"
                        className="group block p-3 rounded-lg hover:bg-white/[0.06] transition-colors"
                        onClick={() => setOpen(false)}
                      >
                        <span className="block text-sm font-medium heading-on-dark group-hover:text-gold-light transition-colors leading-snug">
                          {item.label}
                        </span>
                        <span className="block text-xs text-muted-on-dark mt-1 leading-relaxed line-clamp-2">
                          {item.description}
                        </span>
                      </Link>
                    ))}
                  ...
                  <Link
                    ...
                    role="menuitem"
                    className="mt-4 block p-4 rounded-lg bg-[#07101C] border border-gold/25 hover:border-gold/45 transition-colors group"
                    onClick={() => setOpen(false)}
                  >
